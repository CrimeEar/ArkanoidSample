...
public class BallMovement
{
    ...
    private CircleObject[] AllCircles => _ballObject.AllCircles;
    ...
    private bool HandleCapsuleCollision(Vector2 capsulePosition, float capsuleRadius, float capsuleHeight)
    {
        ...
        if (CheckIntersection(_previousPosition, _newPosition, leftCapCenter + rectNormal * _paddle.Radius, rightCapCenter + rectNormal * _paddle.Radius, out Vector2 intersection))
        {
            _velocity = Vector2.Reflect(_velocity, -rectNormal);
        ...
        if (toLeftCap.magnitude <= _radius + capsuleRadius)
        {
            Vector2 normal = toLeftCap.normalized;
            float overlap = (_radius + capsuleRadius) - toLeftCap.magnitude;
        ...
        if (toRightCap.magnitude <= _radius + capsuleRadius)
        {
            Vector2 normal = toRightCap.normalized;
            float overlap = (_radius + capsuleRadius) - toRightCap.magnitude;
